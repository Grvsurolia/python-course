...
def sqrt():
    global expression
    sq = int(expression)**(1/2)
    equation.set(sq)


def equalpress():
    print("equal press func")
    try:
        global expression
...
button1 = Button(window, text=' 1 ', fg='black', bg='red', height=1, width=7, command=lambda: press(1))
...
sqrt_btn= Button(window, text='sqrt', fg='black', bg='red',
                command=sqrt, height=1, width=7)
sqrt_btn.grid(row=6, column=1)
